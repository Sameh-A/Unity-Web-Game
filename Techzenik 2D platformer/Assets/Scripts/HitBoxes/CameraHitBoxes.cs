...
public class CameraHitBoxes : MonoBehaviour
{  
     public LayerMask whatIsPlayer;
    private SpriteRenderer theSR; 
    public CameraHitBoxes instance;
  ...
  // Start is called before the first frame update
    ...
    void Update()
    {
     if(disableHitCounter<=0){
        ...
        RaycastHit2D hitRightAngle =  Physics2D.Raycast(new Vector3(transform.position.x+(theSR.bounds.extents.x*.45f),transform.position.y+(theSR.bounds.extents.y*.3f),transform.position.z),new Vector2(1,1.75f),.2f,whatIsPlayer);

        RaycastHit2D hitLeftAngle =  Physics2D.Raycast(new Vector3(transform.position.x-(theSR.bounds.extents.x*.45f),transform.position.y+(theSR.bounds.extents.y*.3f),transform.position.z),new Vector2(-1,1.75f),.2f,whatIsPlayer);
    ...
    /*
        ...
        Debug.DrawRay(new Vector3(transform.position.x+(theSR.bounds.extents.x*.45f),transform.position.y+(theSR.bounds.extents.y*.3f),transform.position.z), new Vector2(1,1.75f)*.2f, Color.green);
        Debug.DrawRay(new Vector3(transform.position.x-(theSR.bounds.extents.x*.45f),transform.position.y+(theSR.bounds.extents.y*.3f),transform.position.z), new Vector2(-1,1.75f)*.2f, Color.green);
    */
 


           if ((hitLeft ||hitLeft2 ||hitLeft3 ||hitLeft4 ||hitLeft5 || hitCentre || hitRight || hitRight2 || hitRight3|| hitRight4 || hitRight5 ||hitRightAngle || hitLeftAngle))
        {
